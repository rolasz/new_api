...
import com.example.App_api.repository.RatingRepository;
...
public class RatingService {
    ...
    public Map<String, Object> getThongKe(String id) {
        ...
        for (int i = 0; i < ratings.size(); i++) {
            if (ratings.get(i).getStar() == 1) mot++;
            else if (ratings.get(i).getStar() == 2) hai++;
            else if (ratings.get(i).getStar() == 3) ba++;
            else if (ratings.get(i).getStar() == 4) bon++;
            else nam++;
        }
